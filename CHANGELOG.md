...
- **Extracted testable logic:** Totals, sorting, gauge geometry, duplicate-name checks, row-fit math, JSON validation, and the fetch/save/retry cycle now live in `js/logic.js`, covered by a `node --test` suite (`npm test`) with no new runtime dependencies. Top-level app state (`appData`, `accessToken`, etc.) switched from `let` to `var` so it's a real, introspectable `window` property rather than script-only lexical state — needed for the new DOM-level test suite, and generally useful for debugging.
...
- **DOM-level test suite (`test/dom.test.js`):** Loads `index.html` into `jsdom` and exercises real app functions — tab navigation, dashboard rendering (including an XSS regression check at the DOM level), and the JSON editor's full validate/confirm/save pipeline against a mocked `fetch`.
