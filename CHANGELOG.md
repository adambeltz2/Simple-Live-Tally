...
## [2.0.0] - 2026-09-18
### Changed (Breaking)
- **Rearchitected transaction storage as an append-only ledger, removing the old whole-file edit model.** Previously every transaction lived inline inside the single `data.json` blob, so any add/edit/delete required a fetch-mutate-conditional-save-retry cycle against that one shared file — the more concurrent writers, the more 409 conflicts and retries. Transactions are now individual, immutable files under `/transactions/<eventId>/<id>.json`: adding one writes a `create` entry, editing one writes a `edit` *delta* entry (the numeric difference, not the new total), and deleting one writes a `delete` *negation* entry (its current amount, negated) — nothing is ever overwritten or removed. A transaction's displayed amount is simply the sum of every entry sharing its `logicalId` (`collapseTransactionLedger()` in `js/logic.js`); a `delete` entry drops it from the visible list entirely. Because each entry is its own uniquely-named file (`crypto.randomUUID()`-keyed), two devices can add, correct, or remove transactions at the exact same moment with **zero write conflicts** — there's nothing to retry, since no two writers ever touch the same file. `data.json` (settings/teams/events) keeps the existing fetch/save/409-retry cycle, since it's still genuinely one file multiple people might edit at once. The whole event's ledger is read in a single Dropbox `files/download_zip` call (`downloadTransactionEntries()`), so read cost stays flat regardless of transaction count — up to Dropbox's own ceiling of 10,000 files / 20GB per folder, comfortably beyond what a live event produces. This is a from-scratch data model for a new product with no installed base, so no migration path from 1.x's inline-transaction `data.json` format is provided.
### Added
- **Keyer role (`#keyer`):** A third sign-in mode alongside the existing Admin and TV Viewer. A Keyer session gets the same full Dropbox read/write access as Admin (Dropbox's OAuth scopes can't express a "write-only, path-restricted" grant — see the new README [Roles](README.md#roles) section for the honest limitation here), but the UI is reduced to just the "Add Transaction" panel, with the dashboard/Data Management nav hidden — meant for a dedicated data-entry station run by someone who shouldn't need or see the rest of the admin UI.
- **Device-label attribution:** The first time a `#keyer` device submits a transaction, it's prompted once for a short station label (e.g. "Front Table"), stored in that browser's `localStorage` and stamped onto every entry it creates as `createdBy` (Admin sessions default to `'Admin'` with no prompt). Combined with the immutable ledger, the exported ZIP now shows real per-station accountability for every entry.
- **`localTransactionEntries` optimistic-write buffer:** A queued-but-not-yet-poll-confirmed transaction entry is now shown immediately and merged with the last-fetched ledger (`getVisibleTransactionEntries()`), de-duplicated by id and automatically pruned once a subsequent poll confirms it — fixes a class of bug where an optimistic total could flicker backward if a background poll refresh interleaved with a still-in-flight queued write.
### Removed
- **Bulk "Raw JSON" editor removed entirely**, along with its dedicated Data Management tab, `validateAppDataShape`/`validateAppDataRecords`/`parseAppDataJson` (`js/logic.js`), and its `allowQueue: false` special case. The tool no longer makes sense against an append-only ledger — there's no single JSON blob to bulk-replace, and doing so would defeat the whole point of the immutable transaction history. The full point-in-time ZIP export remains the tool for bulk inspection/audit; see [Full Audit Trail Export](README.md#features).
### Changed
- **"Never destroy ledger data via a UI action":** Factory reset and removing an Event or Team from Data Management now only remove them from the config/active view — the underlying transaction entry files are never deleted, so the complete history always survives and is still included in the ZIP export even after something is removed from the dashboard.
- **README:** Documented the new ledger architecture, the three roles (Admin/Keyer/TV Viewer) and their actual Dropbox-enforced vs. UI-only access boundaries, and the new practical scaling ceiling (Dropbox's own `download_zip` limits, not a homegrown one).
